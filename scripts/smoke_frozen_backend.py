...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ave-release-smoke-") as profile:
        ...
        with tempfile.TemporaryFile(mode="w+b") as log:
            process = subprocess.Popen(args.command, cwd=root, env=env, stdout=log, stderr=log)
            try:
                ...
                while True:
                    ...
                    try:
                        request = Request(f"{base}/api/health", headers={"x-bridge-token": token})
                        ...
                        time.sleep(0.25)
                assert health == {"ok": "true", "service": "automated-video-editing-backend"}, health
                ...
                try:
                    with urlopen(f"{base}/api/health", timeout=5):
                        raise AssertionError("Backend accepted an unauthenticated API request")
                except HTTPError as exc:
                    assert exc.code == 401, exc.code
                print(f"Backend {version} starts, serves health, and rejects requests without its bridge token.")
            except Exception:
                log.seek(0)
